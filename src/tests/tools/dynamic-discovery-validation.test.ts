...
describe('dynamic Discovery Validation', () => {
  ...
  describe('graceful Fallback without n8n', () => {
    ...
    it('should handle failed n8n connection gracefully', async () => {
      ...
      // Connection test may or may not be called depending on discovery mode
      // Key is that it doesn't crash
      expect(tools.some(t => t.name === 'get_system_status')).toBe(true)
    })
  })
  ...
  describe('tool Registry Statistics', () => {
    ...
    it('should track dynamic vs static tools correctly', async () => {
      ...
      expect(staticCount).toBe(6) // Core system tools are static
      expect(dynamicCount).toBeGreaterThan(0) // Should have generated dynamic tools
    })
  })
