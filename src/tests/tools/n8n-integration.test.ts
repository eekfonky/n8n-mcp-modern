...
const skipIfNoN8N = config.n8nApiUrl && config.n8nApiKey ? describe : describe.skip;
...
skipIfNoN8N('N8N Live API Integration', () => {
  ...
  describe('Workflow Operations', () => {
    ...
    it('should activate and deactivate workflow', async () => {
      ...
      let response = await fetch(`${baseUrl}/workflows/${testWorkflowId}/activate`, {
        method: 'POST',
        headers
      });
      ...
      if (response.status === 400) {
        ...
        console.warn('Workflow activation not possible (no trigger nodes) - this is expected for test workflow');
        return;
      }
      ...
      // Deactivate
      response = await fetch(`${baseUrl}/workflows/${testWorkflowId}/deactivate`, {
        method: 'POST',
        headers
      });
  ...
  describe('Execution Operations', () => {
    ...
    it('should execute workflow manually', async () => {
      ...
      const response = await fetch(`${baseUrl}/workflows/${testWorkflowId}/execute`, {
        method: 'POST',
        headers,
  ...
  describe('User and Settings', () => {
    it('should get current user info', async () => {
      ...
      if (response.status === 404 || response.status === 401 || response.status === 400) {
        console.warn('User endpoint not accessible with API key (expected for API-only access)');
        return;
      }
