...
export function loadHandoverConfig(): HandoverConfig {
  ...
  if (process.env.HANDOVER_COMPLEXITY_THRESHOLD) {
    const threshold = parseInt(process.env.HANDOVER_COMPLEXITY_THRESHOLD, 10)
  ...
  if (process.env.HANDOVER_NODE_THRESHOLD) {
    const threshold = parseInt(process.env.HANDOVER_NODE_THRESHOLD, 10)
  ...
  if (process.env.HANDOVER_INTEGRATION_THRESHOLD) {
    const threshold = parseInt(process.env.HANDOVER_INTEGRATION_THRESHOLD, 10)
    if (!isNaN(threshold) && threshold > 0) {
  ...
  if (process.env.HANDOVER_MAX_CHAIN) {
    const maxChain = parseInt(process.env.HANDOVER_MAX_CHAIN, 10)
    if (!isNaN(maxChain) && maxChain > 0 && maxChain <= 10) {
...
export function assessWorkflowComplexity(context: AgentContext): WorkflowComplexity {
  ...
  if (factors.customCodePresent) score += 15
  if (factors.authenticationRequired) score += 10
  if (factors.performanceCritical) score += 15
  if (factors.securitySensitive) score += 15
  
  // Context-based scoring
  if (context.complexity === 'high') score += 20
  else if (context.complexity === 'medium') score += 10
