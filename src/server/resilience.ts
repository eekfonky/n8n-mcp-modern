...
export class CircuitBreaker extends EventEmitter {
  ...
  private onFailure(): void {
    ...
    if (this.state === CircuitState.HALF_OPEN) {
      ...
      logger.warn(`Circuit breaker ${this.name} is now OPEN (will retry at ${this.nextAttempt.toISOString()})`);
...
export class HealthMonitor extends EventEmitter {
  ...
  async performChecks(): Promise<Map<string, HealthCheckResult>> {
    ...
    if (wasHealthy !== this.isHealthy) {
      ...
      logger.info(`System health changed: ${this.isHealthy ? 'HEALTHY' : 'UNHEALTHY'}`);
    }
