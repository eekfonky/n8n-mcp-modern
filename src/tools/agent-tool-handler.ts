...
import { mcpToolProxy, agentToolExecutor } from "./mcp-bridge.js";
...
export class AgentToolHandler {
  ...
  private isToolAllowed(toolName: string): boolean {
    ...
    if (mappedName && this.allowedTools.has(mappedName)) return true;
    
    // Check with MCP prefix
    if (this.allowedTools.has(`mcp__n8n-mcp-modern__${toolName}`)) return true;
