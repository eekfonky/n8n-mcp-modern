...
const GenerateWorkflowSchema = z.object({
  ...
  trigger: z.string().optional().describe('Trigger type (webhook, cron, manual)'),
  integrations: z.array(z.string()).optional().describe('Required integrations/services'),
...
const APIIntegrationSchema = z.object({
  ...
  authType: z.enum(['none', 'api_key', 'oauth2', 'basic_auth']).default('api_key'),
  operations: z.array(z.string()).describe('List of operations (GET, POST, etc.)')
});
...
const NotificationSchema = z.object({
  ...
  channels: z.array(z.string()).describe('Notification channels (slack, email, webhook)'),
  conditions: z.array(z.string()).optional().describe('Conditional logic for notifications'),
...
const WorkflowTemplateSchema = z.object({
  ...
  parameters: z.array(z.string()).optional().describe('Configurable parameters')
});

const DockerComposeSchema = z.object({
  environment: z.enum(['development', 'staging', 'production']).default('development'),
  services: z.array(z.string()).optional().describe('Additional services needed'),
...
const ConditionalLogicSchema = z.object({
  ...
  defaultAction: z.string().optional().describe('Default action if no conditions match')
});
...
const TestingScenariosSchema = z.object({
  workflowId: z.string().describe('Workflow to generate tests for'),
  testTypes: z.array(z.enum(['unit', 'integration', 'end-to-end'])).default(['integration']),
  coverage: z.enum(['basic', 'comprehensive']).default('basic'),
  mockData: z.boolean().default(true)
});

const IntegrationBoilerplateSchema = z.object({
...
export class CodeGenerationTools {
  ...
  static async generateWorkflowFromDescription(args: z.infer<typeof GenerateWorkflowSchema>): Promise<{
    ...
    try {
      await database.recordToolUsage('generate_workflow_from_description', 0, true);
    } catch (error) {
  ...
  static async createAPIIntegrationTemplate(args: z.infer<typeof APIIntegrationSchema>): Promise<{
    ...
    try {
      await database.recordToolUsage('create_api_integration_template', 0, true);
    } catch (error) {
      logger.debug('Database recording skipped:', error);
    }
  ...
  static async buildIntegrationBoilerplate(args: z.infer<typeof IntegrationBoilerplateSchema>) {
    ...
    return { boilerplate: { nodeName: args.nodeName, operations: args.operations } };
  }

  // Helper methods with simplified implementations
  private static extractWorkflowName(description: string): string {
    return description.substring(0, 50).replace(/[^a-zA-Z0-9\s]/g, '').trim() || 'Generated Workflow';
  }

  private static determineTrigger(description: string, trigger?: string): Record<string, unknown> {
    if (trigger) return { type: trigger };
    if (description.includes('webhook')) return { type: 'webhook', method: 'POST' };
    if (description.includes('schedule')) return { type: 'cron', expression: '0 9 * * *' };
    return { type: 'manual' };
  }

  private static async generateNodes(args: z.infer<typeof GenerateWorkflowSchema>): Promise<Record<string, unknown>[]> {
  ...
  private static generateSettings(_args: z.infer<typeof GenerateWorkflowSchema>): Record<string, unknown> {
    return { executionOrder: 'v1', saveManualExecutions: true };
  }

  // Additional helper methods
  private static generateAPINodes(args: z.infer<typeof APIIntegrationSchema>): Record<string, unknown>[] {
    return args.operations.map((op: string) => ({ operation: op, service: args.serviceName }));
  }
  ...
  private static generateEndpointConfigs(operations: string[]): Record<string, unknown>[] {
  ...
  private static generateAPIDocumentation(args: z.infer<typeof APIIntegrationSchema>): Record<string, unknown> {
    return { service: args.serviceName, type: args.apiType };
  }

  private static getImplementationSteps(args: z.infer<typeof APIIntegrationSchema>): string[] {
    return [`Configure ${args.serviceName} credentials`, 'Set up API endpoints', 'Test integration'];
  }

  private static getTestingGuide(args: z.infer<typeof APIIntegrationSchema>): Record<string, unknown> {
    return { steps: ['Unit tests', 'Integration tests'], service: args.serviceName };
  }
  ...
  private static estimateMemoryUsage(_args: z.infer<typeof DataProcessingSchema>): string { 
    return '256MB average'; 
  }
  private static getOptimizationTips(_args: z.infer<typeof DataProcessingSchema>): string[] { 
...
export const codeGenerationTools = [
  {
    ...
    description: 'Generate complete n8n workflow from natural language description',
