...
import type { AgentRoutingRule, TaskComplexity, AgentLoadInfo } from './types.js'
...
class AgentLoadBalancer {
  ...
  selectLeastLoadedAgent(primaryAgent: string, secondaryAgent?: string): string {
    ...
    if (!primaryLoad && !secondaryLoad) return primaryAgent
    if (!primaryLoad) return primaryAgent
    if (!secondaryLoad) return primaryAgent
...
export function getRoutingStats(): {
  ...
  const activeAgents = loadBalancer['agentLoads'].size // Access private member for stats
  
  let totalCapacity = 0
  for (const [, loadInfo] of loadBalancer['agentLoads']) {
