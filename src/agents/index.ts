...
export class MCPOrchestrator {
  ...
  private determineCoordinationType(
    ...
    if (patterns.some((p) => p.frequency > 5)) return "pattern-based";
    return "adaptive";
  }
  ...
  private assessComplexity(
    ...
    if (request.urgency === EscalationUrgency.CRITICAL) return "high";
    if (request.reason === EscalationReason.COMPLEXITY_EXCEEDED) return "high";
...
export class AgentRouter {
  ...
  /**
   * Handle escalation between agents (enhanced with Phase 2 orchestration)
   */
  async handleEscalation(
    sourceAgentName: string,
    escalationRequest: EscalationRequest,
  ): Promise<EscalationResult> {
    // Use new MCP orchestrator for enhanced capabilities
    try {
  ...
  async coordinateWorkflow(
    ...
    for (const capability of requiredCapabilities) {
      ...
      if (capabilityAgents.length > 0) {
        ...
        if (!specialist) continue; // Type guard for safety
    ...
    switch (request.reason) {
      ...
      case EscalationReason.SPECIALIST_KNOWLEDGE_REQUIRED:
        ...
        if (request.requiredCapabilities.length > 0) {
          const capability = request.requiredCapabilities[0];
          if (!capability) break; // Type guard for capability
          const capabilityAgents = this.getAgentsByCapability(capability);
