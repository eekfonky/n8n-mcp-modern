...
class TestRunner {
  ...
  private testSuites: TestSuite[] = [
    {
      name: 'Unit Tests',
      command: 'npx vitest run src/tests/security.test.ts src/tests/tool-execution.test.ts src/tests/tools/',
      timeout: 60000,
      critical: true,
    ...
    },
    {
      ...
      command: 'npx vitest run src/tests/mcp-integration.test.ts src/tests/agent-routing.test.ts src/tests/agents/',
      timeout: 120000,
      critical: true,
  ...
  async validateEnvironment(): Promise<boolean> {
    ...
    const checks = [
      { name: 'Node.js version', check: () => process.version.startsWith('v22') || process.version.startsWith('v20') },
    ...
    const hasN8NCredentials = process.env.N8N_API_URL && process.env.N8N_API_KEY;
    console.log(`   ${hasN8NCredentials ? '✅' : '⚠️ '} N8N credentials (optional for live tests)`);
  ...
  generateReport(): void {
    ...
    this.results.forEach(result => {
      ...
      console.log(`${status} ${critical} ${result.suite} (${result.duration}ms)`);
  ...
  saveJsonReport(): void {
    ...
    const report = {
      ...
      environment: {
        ...
        hasN8NCredentials: !!(process.env.N8N_API_URL && process.env.N8N_API_KEY)
      }
    };
...
Environment Variables:
  ...
  if (args.includes('--coverage')) {
    execAsync('npx vitest run --coverage').then(() => {
      console.log('Coverage report generated');
      process.exit(0);
    }).catch((error) => {
  ...
  if (args.includes('--live')) {
    runner['testSuites'] = runner['testSuites'].filter(s => s.name === 'Live N8N Tests');
  }
