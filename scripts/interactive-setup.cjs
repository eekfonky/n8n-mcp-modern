    ...
    if (scope === 'project') {
      ...
      try {
        ...
        const addCommand = spawn('claude', [
          ...
          '--scope', 'local',
          '--env', `N8N_API_URL=${config.apiUrl}`,
          '--env', `N8N_API_KEY=${config.apiKey}`,
          '--env', `LOG_LEVEL=${config.logLevel || 'info'}`,
