...
class ConfigManager {
  ...
  collectEnvironmentVariables() {
    ...
    if (this.interactive) {
      ...
      const apiUrl = require('child_process').execSync('read -r line; echo "$line"', {
        encoding: 'utf8',
        stdio: ['inherit', 'pipe', 'inherit']
      }).trim();

      process.stdout.write('N8N API Key: ');
      const apiKey = require('child_process').execSync('read -r -s line; echo "$line"', {
        encoding: 'utf8',
        stdio: ['inherit', 'pipe', 'inherit']
      }).trim();

      if (apiUrl) envVars.N8N_API_URL = apiUrl;
      if (apiKey) envVars.N8N_API_KEY = apiKey;
    } else {
      // Non-interactive mode, use environment variables if available
      if (process.env.N8N_API_URL) envVars.N8N_API_URL = process.env.N8N_API_URL;
      if (process.env.N8N_API_KEY) envVars.N8N_API_KEY = process.env.N8N_API_KEY;
    }
